...
nowXX = log('execute')(nowXX)
#我们来剖析上面的语句，首先执行log('execute')，
# 返回的是decorator函数，再调用返回的函数，
# 参数是now函数，返回值最终是wrapper函数。

#以上两种decorator的定义都没有问题，但还差最后一步。
# 因为我们讲了函数也是对象，它有__name__等属性，
# 但你去看经过decorator装饰之后的函数，它们
# 的__name__已经从原来的'now'变成了'wrapper'：
print(nowXX.__name__)

#因为返回的那个wrapper()函数名字就是'wrapper'，
# 所以，需要把原始函数的__name__等属性复制
...
# 模块的概念稍候讲解。现在，只需记住在定义
# wrapper()的前面加上@functools.wraps(func)即可。

'''
在面向对象（OOP）的设计模式中，decorator被称为装饰模式。
OOP的装饰模式需要通过继承和组合来实现，而Python除
了能支持OOP的decorator外，直接从语法层次支持decorator。
...
#---------------practice------------------
#请编写一个decorator，能在函数调用的前后打
# 印出'begin call'和'end call'的日志。
